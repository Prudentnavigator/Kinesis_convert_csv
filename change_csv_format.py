...
def csv_convert(csv, csv_head):
    ''' Convert the Kinesis .csv for Koinly '''
    ...
    with open(csv, "r", encoding="utf-8") as infile:
        for row in infile:
            ...
            # A tuple with the buy amount and buy currency.
            buy = row[6], row[7]

            # A tuple with the sell amount and sell currency.
            sell = row[10], row[9]

            # A tuple with the fee amount and fee currency.
            fee = row[11], row[12]
...
def main():
    ...
    # Store the platform and header in a tuple.
